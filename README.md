...
- ピラミッド犬は勝手にぴょこぴょこ歩き回ったり、ひるねしたり、ときどき走り回ったり(ズーミー)、
  ごろんと転がったり、ちょうちょを追いかけたりします
- **タップ / クリック** … ジャンプしてよろこびます(鳴き声つき)
- **ながおし** … なでなで。なでる場所(あたま/かお/からだ)で反応が変わります
- **ドラッグ** … つかんで持ち上げてお引っ越し。はなすとふわっと着地します
- **😇/😈** … いやしモード(あまえんぼう)⇄ どくぜつモード(ツンデレ)を切り替えます
- **👀 こっちむいて** … こちらを向いて首をかしげます
- **🐾 おいで** … カメラの近くまで来てくれます
- **🎾 ボール** … ボールを投げると走って くわえて 持ってきてくれます(とってこい)
- **🍎 おやつ** … 押すたびに りんご/ほね/クッキー を切り替えてあげられます
- **🐶➕ ふやす / ちび** … なかまを追加(通常5匹+ちび5匹まで)
- **📷** … その瞬間のスクリーンショットを保存
- 遊ぶほど **なつき度(💛)** が上がります。時間帯(朝・昼・夕・夜)で空や BGM が変化し、夜は星が出ます
- ホーム画面に追加すると **PWA** としてオフラインでも遊べます
- たまに吹き出しで話しかけてきます

> 設計の全体像は [DESIGN.md](DESIGN.md) / 改善案と実装状況は [ROADMAP.md](ROADMAP.md) を参照。
> このブランチのモデルは Tripo3D 製で、手編集済みの `blender/piramidog_tripo_self.blend` から書き出しています。
- ドラッグでカメラを回転、ピンチ/ホイールでズームできます
- 右上の 🔊 ボタンで BGM・効果音のオン/オフ
- 左下のボタン群は 🐾 ボタンで開閉できます(操作しないと8秒で自動的にしまわれます)
- ❓ボタンでいつでも操作方法を見られる
